...
(*
  Classes is basically a code telling to determine if the ast is `term`,
  `type` or a `kind`. The original paper says that classes let us approximate
  the types of terms.
*)

(* 
  You are either a prop (which may refer to `prop` sort as well as to
  both `set` and `prop. I assume it's both of them) or branch. We represent
  that idea with this tree.

  The skeletons later play the major role in the strong normalization
  propperty and are used to encode `orders` of schemes.
*)
Inductive skel : Type :=
| PROP : skel
| PROD : skel -> skel -> skel
...
(* Skeleton equality is decidable *)
...
(* 
  The class of a term. It's either a `Term`, a `Type` or
  a `Kind`
*)
...
(*
  This function extracts skeletons from `Kinds` and maps
  anything else to `PROP`
*)
...
(*
  This one extracts skeletons from `Types` and maps
  anything else to `PROP`
*)
...
(* A small lemma for "pushing" a function inside a match statement *)
Lemma commut_case :
...
(* 
  A procedure which computes the class of a term. We'll later prove its
  correctness 
*)
Fixpoint cl_term (t : term) : cls -> class :=
  ...
  | NatO => Trm
  | NatSucc _ => Trm
  | NatDestruct _ _ _ _ => Trm
  | NatCases _ _ _ _ => Trm
  end
.


(* 
  This function computes the class-context of
  a normal context
*)
...
(*
  A theorem about nth item of a list.
*)
Lemma nth_class_env :
...
(* 
  This lemma is about extending the environment and
  preserving the term's class
*)
Lemma cl_trunc :
  forall (n : nat) (e f : cls),
  TTrunc _ n e f -> 
  forall t : term, cl_term (lift n t) e = cl_term t f
.
Proof.
...
(* 
  Loose equality of two classes is pretty equivalent to 
  normal class equality with the excpetion that skeletons
  of `Types` can be ignored.
*)
...
Lemma loose_eqc_trans :
  forall c1 c2 : class,
  loose_eqc c1 c2 -> 
  forall c3 : class, loose_eqc c2 c3 -> loose_eqc c1 c3
.
Proof.
...
(* Adjacency relation of two the classes *)
Inductive adj_cls : class -> class -> Prop :=
| adj_t : forall s : skel, adj_cls Trm (Typ s) (* `Term` is adjencent with `Type` *)
| adj_T : forall s1 s2 : skel, adj_cls (Typ s1) (Knd s2) (* `Type` and adjencent with `Kind` *)
.
...
(* 
  If two environments are loosly equal the classes we infer 
  under these two environments for the same term are loosely
  equivalent. We say that the classes are loosely stable.
*)
...
(*
  This is a substitution lemma. Here we require for the value we substitute
  with to have an adjecent class with the class of the term where the substituion
  happens. We state that the resulting class we get is loosely preserved.
*)
Lemma cl_term_subst :
...
(* 
  If one can infer that a term has type `Kind`, we assert that the projections 
  work as expected.
*)
Lemma class_knd :
...
(* The same theorem for `Type` class *)
Lemma class_typ :
  forall (e : env) (t T : term),
  typ e t T ->
  typ e T (Srt kind) ->
...
(*
  We use these two lemmas to prove the following "induction principle"
  for a term which has some sort as its type.
*)
Corollary class_typ_ord :
...
(* Then a simillar lemma for `Term` class *)
Lemma class_trm :
  forall (e : env) (t T : term) (s : sort),
  is_prop s -> 
  typ e t T -> typ e T (Srt s) -> 
...
Proof.
  ...
  exfalso; generalize is_p; cut (Srt s = Srt kind).
  intros F; injection F; intros; subst s.
  destruct is_p0; congruence.
  apply type_kind_not_conv with e0 (Srt set).
  destruct s; auto with coc core.
  apply type_set; auto with coc core.
...
(* 
  We prove that our class theory is sound by proving the following:

  If a term `t` has type `T` under environment `e`, then the classes
  of `t` and `T` are ajacent. That sounds... Quite sound.
*)
Lemma cl_term_sound :
  forall (e : env) (t T : term),
  typ e t T ->
  forall K : term, typ e T K -> 
...
(* Thhe one-step reduction loosely preserves the class *)
Lemma cl_term_red1 :
...
Proof.
  ...
  simpl in |- *.
  inversion H8.
  subst B; auto with coc core.
  subst B; auto with coc core.
  subst B; auto with coc core.
  subst B; auto with coc core.
  subst B.
  rewrite class_trm with e0 x2 (App x1 NatO) set; auto with coc core.
  unfold is_prop.
  right; reflexivity.
  change (Srt set) with (subst NatO (Srt set)).
  apply type_app with Nat; auto with coc core.
  apply type_nat_o; eapply typ_wf; eauto with coc core.
  subst B.
  rewrite class_trm with e0 (App x3 num) (App x1 (NatSucc num)) set; auto with coc core.
  unfold is_prop.
  right; reflexivity.
  rewrite <- (lift0 num).
  ...
  change (Srt set) with (subst (NatSucc num) (Srt set)).
  apply type_app with Nat; auto with coc core.
  subst x4; auto with coc core.
  ...
  subst B; auto with coc core.
  subst B.
  rewrite class_trm with e0 x2 (App x1 NatO) prop; auto with coc core.
  unfold is_prop.
  left; reflexivity.
  ...
  apply type_nat_o; eapply typ_wf; eauto with coc core.
  subst B.
  rewrite class_trm with e0 (App x3 num) (App x1 (NatSucc num)) prop; auto with coc core.
  unfold is_prop.
  left; reflexivity.
  rewrite <- (lift0 num).
  rewrite <- (subst_ref_eq num 0) at 2.
  ...
  apply type_conv with Tx set; auto with coc core datatypes.
  apply type_nat; eapply typ_wf; eauto with coc core datatypes.
  change (Srt prop) with (subst (NatSucc num) (Srt prop)).
  apply type_app with Nat; auto with coc core.
  subst x4; auto with coc core.
Qed.
...
(* Multistep reduction loosely preserves the class *)
Lemma cl_term_red :
  forall T U : term,
  red T U ->
  forall (e : env) (K : term),
...
(* The skeletons of the term's and its type's class are the same *)
Lemma skel_sound :
  forall (e : env) (t T : term),
  typ e t T ->
...
Proof.
  ...
  (commut_case _ _ cv_skel)
  .
  destruct (cl_term x1 (class_env e0)); auto with coc core.
  rewrite <- H1.
  reflexivity.
...
(* A strange predicate which resembles adjency of classes *)
...
(* Substitution preserves the class *)
Lemma class_subst :
  forall (a : class) (G : cls) (x : term),
...
(* We now show that class inference obeys the pattern described by `typ_cls` *)
Lemma class_sound :
  forall (e : env) (t T : term), typ e t T ->
  forall K : term, typ e T K -> 
...
(* Reduction preserves the term's class *)
Lemma class_red :
