...
(*
  The definitions we've done call for some stability results.
*)
...
(*
  We show that interpretation of a type is an invariant
*)
Lemma int_equiv_int_typ :
  forall (T : term) (i i' : intP), int_eq_can i i' ->
...
(* 
  If all variables are interepted by an invariant predicate, the
  then the interpretation of any type is an invariant.
*)
Lemma int_typ_cr :
...
(*
  Interpretations are stable (equal) with insertion.
*)
Lemma nth_lift_int :
...
(* Interpretations are stable by lifting *)
Lemma lift_int_typ :
...
(* The classes are preserved with lifting *)
...
Proof.
  ...
  simpl in H3.
  exfalso; exact (H3 (eq_refl _)).

  simpl in H3.
  exfalso; exact (H3 (eq_refl _)).

  simpl in H5.
  exfalso; exact (H5 (eq_refl _)).
